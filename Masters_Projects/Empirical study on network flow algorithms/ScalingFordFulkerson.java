...
public class ScalingFordFulkerson {
    ...
    public double computeMaxFlow() {
        ...
        int delta = (int) Math.pow(EXPONENTIAL_BASE, computeLogBase2(graph.getMaxEdgeCapacity()));

        while (delta >= 1) {
            while (doesAugmentingPathExists(graph, s, t, delta)) {
    ...
    private int computeLogBase2(double x) {
    ...
    private boolean doesAugmentingPathExists(SimpleGraph graph, Vertex s, Vertex t, int delta) {
        ...
        while (!queue.isEmpty() && !isVisited(t)) {
            ...
            for (Edge edge : fromVertex.incidentEdgeList) {
                ...
                if (computeResidualCapacityToVertex(edge, toVertex) >= delta) {
